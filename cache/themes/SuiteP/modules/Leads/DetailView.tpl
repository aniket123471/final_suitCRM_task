...
{capture name="label" assign="label"}{sugar_translate label='LBL_FOLLOWUP_DATE' module='Leads'}{/capture}
{$label|strip_semicolon}:
</div>
...
{if !$fields.followup_date.hidden}
{counter name="panelFieldCount" print=false}
...
{else}
{if strlen($fields.followup_date.value) <= 0}
{assign var="value" value=$fields.followup_date.default_value }
{else}
{assign var="value" value=$fields.followup_date.value }
{/if}
{/if}
<span class="sugar_field" id="{$fields.followup_date.name}">{$value}</span>
{/if}
